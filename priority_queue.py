class PriorityQueue(object):
    ...
    def pop(self):
        ...
        # 3) Bubble down until no more swapping occurs
        i = 0
        i_left = (i * 2) + 1
        i_right = (i * 2) + 2
        while (i_left < self.size()) or (i_right < self.size()):
            i_swap = i
            # Locate the smallest node out of parent, left child, and right child
    ...
    def append(self, node):
        ...
        # Bubble up the node until no swapping occurs
        i = self.size() - 1
        i_parent = (i - 1) // 2
    ...
    def compare(self, a, b):
        ...
        if self.queue[a][1][0] < self.queue[b][1][0]:
            return True
        elif (self.queue[a][1][0] == self.queue[b][1][0]) and (
            self.queue[a][0] < self.queue[b][0]
        ):
            return True
        else:
            return False
